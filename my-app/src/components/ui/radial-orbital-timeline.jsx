...
export default function RadialOrbitalTimeline({
  ...
  useEffect(() => {
    // Auto-open each card sequentially with 3-second delay between each
    let currentIndex = 0;
    const timers = [];

    const openNextCard = () => {
      if (currentIndex < timelineData.length) {
        const cardId = timelineData[currentIndex].id;
        
        setExpandedItems((prev) => ({
          ...prev,
          [cardId]: true,
        }));
        setActiveNodeId(cardId);
        
        // Close this card after 2.5 seconds, then open next
    ...
    const startTimer = setTimeout(openNextCard, 500);
  ...
  const calculateNodePosition = (index, total) => {
    ...
    const isHovered = hoveredItemId === timelineData[index].id;
    const opacity = hoveredItemId === null ? 0.3 : (isHovered ? 1 : 0.2);
  ...
  return (
    <div
      ...
      <div
        className="relative w-full max-w-4xl h-full flex items-center justify-center z-10">
        <div
            ...
            return (
              <div
                ...
                {(isExpanded || hoveredItemId === item.id) && (
                  <Card
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 bg-black/90 backdrop-blur-lg border-white/30 shadow-xl shadow-white/10 overflow-visible z-50">
                    <div
